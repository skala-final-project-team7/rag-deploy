...
## 4b. backend-template 동기화 (2026-06-11) — api-spec v2.6.2 (site_url 단일화·§2-5 `siteUrl`)
backend-template 2026-06-11 확정을 문서에 반영했다(`docs/api-spec.md` v2.6.2, ADR 0001 §3, db-schema):

- **site_url 단일화**: BE DB 는 `admin_atlassian_credential.site_url` 한 곳에만 저장(`public_site_url` 별도 명칭 없음). §2-5 credential lookup 응답 = `{accessToken, cloudId, siteUrl, expiresAt}`.
- **rag 코드 무변경**: `sources[].url` 은 색인 payload `webui_link` passthrough(`rerank_node.confluence_url`)라, ingestion-deploy 가 `siteUrl` 기준 absolute 로 정규화 적재하면 그대로 absolute 가 노출된다(rag 측 수정 불필요). **단, 기존 색인분은 상대경로이므로 ingestion-deploy 반영 후 재색인(full ingest) 필요.**
- `/ml/query` 계약(v2.6.0 — `userId`=accountId fail-closed·`groups`=groupId 빈 배열 허용·spaceKey 없음)은 기구현 정합 확인.
...
- `docs/api-spec.md` (v2.6.2) — 정본 계약 (backend-template `docs/api-spec.md` 2026-06-11 자와 동기)
