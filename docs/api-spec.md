...
>
> **수집 자격증명 모델 (2026-06-11 정정 — Feature 0 게이트 결론)**: admin 도 일반 사용자와 동일하게 Confluence OAuth 3LO 로 **로그인**하지만(§4-1 — LINA JWT 발급 용도), admin-key 의 발급/사용은 OAuth 로 불가하다(공식: Confluence admin-key REST 리소스는 Forge/OAuth2/Connect 앱 접근 불가). 따라서 ingestion 의 admin-key 경로는 **admin API Token**(id.atlassian.com 에서 수동 발급한 정적 단일 credential)을 사용한다 — 2026-06-02 의 "같은 admin OAuth access_token 사용(별도 API Token 미사용)" 모델은 **폐기**. page-level read restriction 우회를 위해 Atlassian **Admin Key** 가 활성화되어 있어야 하고, Data Ingestion Pipeline 이 admin-key 가 적용되는 Atlassian REST 호출 시 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 헤더를 **site URL** 에서 부여한다(OAuth 게이트웨이 경로에서는 admin-key 미동작). 자세한 ACL 적재 흐름은 `docs/adr/0001-page-level-acl-source.md` §2.1 참조.
>
...
Request Body 없음 (admin API Token 은 서버 측에서 보관·사용 — 본문/응답에 노출하지 않음).
...
- BFF → auth-server 내부 API 호출 → auth-server 가 보관된 **admin API Token** 으로 site URL 의 Atlassian `POST /wiki/api/v2/admin-key` 활성화 (`Authorization: Basic base64(adminEmail:adminApiToken)` — admin-key 발급/사용은 OAuth 불가, 2026-06-11 정정)
- 응답 `activatedUntil` 을 FE 가 표시(만료 시각·count-down). 만료 후 admin 이 재활성화

> **✅ 검증 게이트 결론 (Feature 0, 2026-06-11 확정):** OAuth Bearer + `Atl-Confluence-With-Admin-Key: true` 조합은 **불가** — Confluence admin-key REST 리소스는 Forge/OAuth2/Connect 앱이 접근할 수 없다(공식). 따라서 fallback 으로 두었던 **admin API Token 분리 보관 모델을 채택**한다: `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true`, admin-key 적용 read 는 site URL 에서 수행. 팀 사전 테스트(`confluence_admin_key_test_summary.md`, 2026-06-02)의 API Token + Admin Key 조합 동작 확인과 정합.
...
4. Data Ingestion Worker 는 job 을 consume한 뒤 auth-server 내부 credential 조회 API 로 `adminUserId` 기준 **admin API Token + adminEmail + cloudId** 를 함께 조회한다 (2026-06-11 정정 — OAuth `accessToken` 아님. 저장/조회 위치(DB vs env Secret)는 별도 결정). `cloudId` 는 Confluence 호출/대상 site 지정에 사용한다.
5. Data Ingestion Worker 는 admin-key 가 적용되는 Confluence REST 호출 시 **`Authorization: Basic base64(adminEmail:adminApiToken)`** + `Atl-Confluence-With-Admin-Key: true` 헤더를 **site URL 에서** 사용한다 (2026-06-11 정정 — OAuth Bearer 로는 admin-key 미동작, 위반 시 restricted 페이지 무음 누락).
...
- `adminUserId`: auth-server 에 저장된 admin Confluence credential 을 조회하기 위한 사용자 식별자. credential 자체가 아니다.
- `adminApiToken` / `accessToken` / `refreshToken` / `cloudId` 는 본문에 포함하지 않는다. Data Ingestion Worker 는 job consume 후 auth-server 내부 credential 조회 API 로 **admin API Token + adminEmail + cloudId** 를 함께 조회한다(§2-5, 2026-06-11 정정). admin-key 가 적용되는 Confluence REST 호출에는 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 를 site URL 에서 사용한다.

> **RabbitMQ job payload 원칙:** `/ml/ingest` HTTP 호출이 내부적으로 MQ job 을 발행하든 BFF 가 직접 RabbitMQ 에 발행하든, MQ payload 는 작업 식별/상태 정보만 포함한다. `cloudId` 는 payload 로 전달하지 않고 auth-server 내부 credential 조회 응답에서 `adminApiToken`·`adminEmail` 과 함께 반환된다(§2-5).
...
| 설명   | **admin API Token + adminEmail + cloudId** 를 함께 조회 (2026-06-11 정정 — OAuth `accessToken` 아님) |
...
```json
{
  ...
  "expiresAt": "2026-12-05T20:00:00+09:00"
}
```

- OAuth `accessToken` / `refreshToken` 은 응답하지 않는다 (2026-06-11 정정 — admin-key 경로는 OAuth 미사용. OAuth 3LO 는 로그인/LINA JWT 발급 용도로만 유지).
- `adminApiToken` 은 id.atlassian.com 에서 수동 발급한 **정적 단일** credential 이다. OAuth refresh 흐름이 없으며, 만료(`expiresAt` — 발급 시 설정된 토큰 만료일)·회전 시 재발급 후 auth-server 보관본을 교체한다. 저장/조회 위치(DB vs env Secret)는 별도 결정(본 정정 범위 밖).
- auth-server 는 `adminUserId` 로 사용자/credential 레코드를 조회하고 `users.role == ADMIN` 을 확인한다.
- `cloudId` 는 RabbitMQ payload 가 아니라 이 응답에서 `adminApiToken`·`adminEmail` 과 함께 반환된다. Worker 가 Confluence 호출/대상 site 지정에 사용한다.
- Data Ingestion Worker 는 admin-key 가 적용되는 Confluence REST 호출 시 `Authorization: Basic base64(adminEmail:adminApiToken)` + `Atl-Confluence-With-Admin-Key: true` 헤더를 **site URL 에서** 사용한다 (OAuth Bearer/게이트웨이 경로에서는 admin-key 미동작).
