...
| 3 | `verification` | 0~1회 | **JSON 객체** (집계) | `JSON.parse` 필요. **검색 0건(RETRIEVAL_EMPTY)이면 생략**(api-spec §1-1 "0건 처리" — 검증할 근거 없음) |
...
추가로, 진행 표시용 `status` 이벤트(feature19)가 위 핵심 이벤트들 사이사이에 끼어
들어온다(**양 모드 공통** — 2026-06-10 A14 반영). `status` 는 *추가 전용* 이벤트라, 이를
무시하는 클라이언트는 핵심 5종만으로 기존과 동일하게 동작한다. 스트리밍 모드는 phase 7종을
모두 거치고, 비스트리밍(PoC fallback 포함)은 단일 invoke 구조라 `connecting` →
`acl_filtering` → `searching` → `formatting` 4종으로 축약 송신된다(아래 3.6 / 8번 참고).
...
- **token 은 append-only 다** — 수신 순서대로 그대로 이어 붙인다(정본 api-spec §1-1, BFF
  ChatService 동작과 일치). 클리어/덮어쓰기 시맨틱은 없다(2026-06-10 A5 정리 — 아래 6번).
...
- api-spec v2.2.0 §1-1 정합. `title`(LLM 생성 대화 제목)은 스펙상 optional 이며 **본 ML 구현은
  생성해 송신한다**(2026-05-29 titler 도입 — 운영: GPT-4o-mini 요약, PoC/실패 시: 질문 앞부분
  결정론 fallback. `app/query/titler.py`, `routes._resolve_title`). FE 는 `title` 이 없으면
  대화 제목을 갱신하지 않는다(생성 실패 시 생략될 수 있는 optional 계약은 동일).
...
스트리밍 정상 흐름 순서: `connecting` → `acl_filtering` → `searching` → `answering` →
`streaming` → `verifying` → `formatting`. 검색 0건(`RETRIEVAL_EMPTY`) 분기와
**비스트리밍 경로(PoC fallback 포함 — A14)** 는 `answering`/`streaming`/`verifying` 를
건너뛰고 `searching` 다음 `formatting` 으로 직행한다(4종 축약). `done`/`error` 는
...
## 6. 스트리밍 모드(`stream=true`) 특수 동작

> **2026-06-10 A5 정리** — 종전의 "빈 token=클리어"·"차단문 token 재전송(덮어쓰기)" 시맨틱은
> **제거됐다**. backend-template ChatService(@eafd6b3) 확인 결과 BFF 는 token.content 를
> append-only 로 누적·영속하며 클리어/교체를 구현하지 않는다(정본 api-spec §1-1 과 동일).
> ML 도 append-only 만 전제한다. 프론트는 token 을 이어 붙이기만 하면 된다.

1. **Rate Limit fallback — 구분 안내문 token**
   OpenAI Rate Limit 발생 시 fallback 모델로 재시도한다. 부분 답변이 이미 송신된 경우
   `"\n\n(일시적인 응답 한도로 답변을 처음부터 다시 생성합니다)\n\n"` 안내문 token 1회 후
   재생성 답변 token 이 이어진다 — 그대로 누적하면 자연스러운 본문이 된다(빈 token 없음).

2. **차단 분기 — token 재전송 없음, `verification` 으로 판단**
   스트리밍 종료 후 답변이 차단 분기(NOT_SUPPORTED 비율 > 0.5)로 판정되어도 token 은 더
   오지 않는다. 직후 `verification` 이벤트가 `verificationResult: "NOT_SUPPORTED"` 와 낮은
   `confidenceScore` 를 전달하므로, 차단 상태 표시는 이 이벤트 기반으로 렌더링한다(정책은
   FE 소관 — BFF 는 confidence/result 를 그대로 영속). 비-streaming(`stream=false`)은 검증
   후 단일 token 이라 종전대로 차단 안내문이 답변 본문으로 온다.
...
- **진행 상태(progress) `status` 이벤트는 양 모드 모두 송신된다**(feature19 + 2026-06-10
  A14). 실 스트리밍(OpenAI 키 연동) 경로는 phase 7종 전부, 비스트리밍·PoC fallback 경로는
  4종 축약(`connecting`/`acl_filtering`/`searching`/`formatting`)으로 push 된다(위 3.6).
- 상태성 정보는 `verification.confidenceScore`/`verificationResult`, `meta.feedback_enabled`,
  진행 `status.phase`, 에러 `errorCode` 로 나뉘어 있다.
